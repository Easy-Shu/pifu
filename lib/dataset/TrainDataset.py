...
class TrainDataset(Dataset):
    ...
    def get_render(self, subject, num_views, yid=0, pid=0, random_sample=False):
        ...
        # eg. num_views=1, view=yaw; num_views=2, view1=yaw, view2=yaw+180
        ...
        for vid in view_ids:
            param_path = os.path.join(self.PARAM, subject, '%d_%d_%02d.npy' % (vid, pitch, 0))  # calib setting
            render_path = os.path.join(self.RENDER, subject, '%d_%d_%02d.jpg' % (vid, pitch, 0))  # rendered picture
            mask_path = os.path.join(self.MASK, subject, '%d_%d_%02d.png' % (vid, pitch, 0))  # 0/1 mask
            ...
            # 这里的extrinsic的目的是把物体旋转后移到远点
            # 因此translate计算的是物体旋转后，物体中心到原点的位移
            ...
            # intrinsic的目的是把物体从相机坐标归一化到world世界中的坐标（物体中心处于零点）
    ...
    def select_sampling_method(self, subject):
        """
        从mesh中随机取点，返回这些点和他们对应的label，即是否在物体内
        取点的方法有两种：表面取点和随机取点，数量比例是16：1，在表面取点中，会加上随机噪声
        """
        ...
        inside_points = inside_points[:self.num_sample_inout // 2] \
            if nin > self.num_sample_inout // 2 else inside_points
        outside_points = outside_points[:self.num_sample_inout // 2] \
            if nin > self.num_sample_inout // 2 else outside_points[:(self.num_sample_inout - nin)]
    ...
    def get_item(self, index):
        ...
        sid = index % len(self.subjects)  # subject idx
        tmp = index // len(self.subjects)
        yid = tmp % len(self.yaw_list)  # yaw index (left/right)
        pid = tmp // len(self.yaw_list)  # pitch index (up/down)
        ...
        render_data = self.get_render(
            subject, num_views=self.num_views, yid=yid, pid=pid, random_sample=self.opt.random_multiview)
